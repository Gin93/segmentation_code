...
filename = 'F:\Segmentation code\segmentation_code\timestamp\segmentation_thermal.csv';
...
for i = 3:length( all_files) % for each participant , from 1-51 , No participant = i - 2
    ...
    for j = 1:length(all_videos)
        ...
        if length(strfind(file_name , '.ats')) == 1 % the file_name is a .ats file
            ...
            %% set parameters
            %             cur_ts = ts(1); %read the current ts and attribute, remove them from the list then,
            %             cur_atr = atr_names(1); % so that, only need to read the first one in the list each time
            %             ts(1) = [];
            %             atr_names(1) = [] ;
            ts = [10, ts];
            ...
            if ismember (participant_index , [31])
                ...
                while ~isDone(v)
                    ...
                    if   frame_timestamp > next_ts % save this, read next VALID one;
                        ...
                        output_path = join(['f:\output video jin\thermal3\' , output_file_name],'');
                        output_path_fc = join(['f:\output video jin\thermal3\' , output_file_name_fc],'');
