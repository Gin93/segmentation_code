...
filed_names = txt_field(1,2:13);

%%
for i = 27:length( all_files) % for each participant , from 1-51 , No participant = i - 2
    %     all_files(i).name
        ...
        if length(strfind(file_name , '.ats')) == 1 % the file_name is a .ats file
            ...
            while ~isDone(v)
                ...
                if   frame_timestamp > next_ts % save this, read next VALID one;
                    ...
                    output_path = join(['G:\output video jin\thermal3\' , output_file_name],'');
                    output_path_fc = join(['G:\output video jin\thermal3\' , output_file_name_fc],'');
